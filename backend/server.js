...
// In-Memory Database fallback for initial landing page testing
const MOCK_CAMPAIGNS = [
  {
    ...
    milestones: [
      { id: 1, title: 'MVP Launch', status: 'done' },
      { id: 2, title: 'First 100 Users', status: 'active' },
      { id: 3, title: 'Revenue ৳50K', status: 'locked' }
    ],
    verified: true,
  ...
  },
  {
    ...
    milestones: [
      { id: 1, title: 'Hub Setup in Banani', status: 'done' },
      { id: 2, title: 'First 500 Deliveries', status: 'done' },
      { id: 3, title: 'Automated Routing App', status: 'active' }
    ],
    verified: true,
...
app.post('/api/vetting/apply', (req, res) => {
  const { name, university, nid, mfsNumber, role } = req.body;
  ...
  res.status(201).json({
    message: 'Identity data registered in vetting queue.',
...
app.get('/api/milestones/:campaignId', (req, res) => {
